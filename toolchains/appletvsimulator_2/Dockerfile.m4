...
ENV TARGET_DIR=/opt/appletvsimulator

ENV SDK_DIR=/opt/sdk

# Keep in sync with apple_sdks
COPY --from=sdks /sdk/AppleTVSimulator`'TVOS_SDK_VERSION`'* ${SDK_DIR}/
...
COPY appletvsimulator.platform ${TARGET_DIR}/
...
helpers_package(libjpeg-turbo, -DCMAKE_TOOLCHAIN_FILE=${TARGET_DIR}/appletvsimulator.platform -DREQUIRE_SIMD=0)
...
helpers_package(curl, --without-ssl --with-secure-transport --disable-ntlm-wb, CFLAGS="-mtvos-simulator-version-min=TVOS_DEPLOYMENT_TARGET")
