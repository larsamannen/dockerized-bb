...
m4_define(`IPHONE_SDK_VERSION',16.1)m4_dnl
...
ENV TARGET_DIR=/opt/iphonesimulator

ENV SDK_DIR=/opt/sdk

# Keep in sync with apple_sdks
COPY --from=sdks /sdk/iPhoneSimulator`'IPHONE_SDK_VERSION`'* ${SDK_DIR}/
...
ENV HOST=aarch64-apple-darwin11 \
	PREFIX=${TARGET_DIR}/SDK/iPhoneSimulator`'IPHONE_SDK_VERSION`'.sdk/usr
...
helpers_package(curl, --without-ssl --with-secure-transport, CFLAGS="-miphonesimulator-version-min=IPHONEOS_DEPLOYMENT_TARGET")
