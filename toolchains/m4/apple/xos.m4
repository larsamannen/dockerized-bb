...
helpers_package(bzip2, , XOS_C_LD_FLAGS)

helpers_package(libpng1.6, , XOS_C_LD_FLAGS)

helpers_package(libjpeg-turbo, -DREQUIRE_SIMD=0 -DCMAKE_TOOLCHAIN_FILE=${TARGET_DIR}/XOS_PLATFORM`'.platform)

helpers_package(giflib, , XOS_C_CPP_LD_FLAGS)

helpers_package(faad2, , XOS_C_LD_FLAGS)

# Fork is not supported in tvOS and not used in iOS
helpers_package(libmad, ac_cv_func_fork=no, XOS_C_LD_FLAGS)

helpers_package(libogg, , XOS_C_LD_FLAGS)

helpers_package(libtheora, , XOS_C_LD_FLAGS)

helpers_package(libvorbis, XOS_C_CPP_LD_FLAGS)

helpers_package(flac, , XOS_C_LD_FLAGS)
...
helpers_package(mpeg2dec, , XOS_C_LD_FLAGS)

helpers_package(a52dec, , XOS_C_LD_FLAGS)
...
helpers_package(curl, --without-ssl --with-secure-transport --disable-ntlm-wb, CFLAGS="-m`'XOS_PLATFORM`'os-version-min=XOS_DEPLOYMENT_TARGET XOS_BITCODE_FLAG")

helpers_package(freetype, , XOS_C_LD_FLAGS)

helpers_package(fribidi, , XOS_C_LD_FLAGS)

# Don't depend on SDL2 (paradoxical)
COPY --from=apple-common /lib-helpers/packages/libsdl2-net lib-helpers/packages/libsdl2-net
helpers_package(libsdl2-net, , XOS_C_LD_FLAGS)

# Fluidsynth depends
helpers_package(ffi, , XOS_C_LD_FLAGS)

helpers_package(intl, , XOS_C_LD_FLAGS)

# Lighten glib build by removing Objective C and Cocoa and fix intl detection
COPY --from=apple-common /lib-helpers/packages/glib2.0 lib-helpers/packages/glib2.0
helpers_package(glib2.0, XOS_MESON_FLAGS)
